...
export function CreateProfile({ onSuccess }: CreateProfileProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check for Mock Mode
      const { IS_MOCK_MODE } = await import("../config/sui");

      if (IS_MOCK_MODE || !PACKAGE_ID || !REGISTRY_ID) {
        console.warn("⚠️ MOCK MODE ENABLED: Skipping on-chain registration.");
        alert("✅ Example Profile created (Mock Mode)!");
